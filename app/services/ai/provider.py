...
from app.services.ai.base import IcebreakerProvider, TagInsightProvider, WelcomeProvider
...
_welcome_provider: WelcomeProvider | None = None
...
def get_welcome_provider() -> WelcomeProvider:
    global _welcome_provider
    if _welcome_provider is None:
        if settings.gemini_api_key:
            from app.services.ai.gemini_provider import GeminiWelcomeProvider

            _welcome_provider = GeminiWelcomeProvider()
        else:
            _welcome_provider = TemplateWelcomeProvider()
    return _welcome_provider
